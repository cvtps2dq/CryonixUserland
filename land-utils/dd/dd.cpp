...
void transfer_data(const Config& cfg) {
    const int in_fd = open(cfg.input.c_str(), O_RDONLY);
    if (in_fd < 0) {
        std::cerr << Colors::RED << "Failed to open input: " << cfg.input << "\n" << Colors::RESET;
        return;
    }

    const int out_fd = open(cfg.output.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0666);
    if (out_fd < 0) {
    ...
    const double speed = total_bytes / (elapsed * 1024 * 1024);
